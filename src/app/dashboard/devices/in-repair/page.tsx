...
export default function InRepairPage() {
  const [devices, setDevices] = useState<Device[]>([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState('');
  const [deleteId, setDeleteId] = useState<string | null>(null);
  useEffect(() => {
    fetch('/api/devices').then((r) => r.json()).then((d) => { setDevices((d.devices || []).filter((x: Device) => x.status === 'Inactive')); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className='space-y-6'>
      <PageHeader title='Devices In Repair' description='Assets currently undergoing maintenance or repair.' backHref='/dashboard/devices' action={{ label: 'Add Device', href: '/dashboard/devices/add' }} />
